...
def eval(whiteParam, blackParam, kingParam, colorParam):
    weights = [10, 8, 7, 20, timeRemaining / 10, 9]
...
whitePieces = 0b11110000011110000
blackPieces = 0b1111000001111000000000000000000
kingPieces = 0b1001000000110001100000010010000
pieceColor = "WHITE"
timeRemaining = 100.0
maxDepth = 6
...
for i in range(0, int(maxCount)):
    startTime = time.time()
    maxMove(whitePieces, blackPieces, kingPieces, pieceColor,  float('-inf'), 
            float('inf'), 1, "")
    sumTime += (time.time() - startTime)

f = open("calibrate.txt", 'w')
f.write(str(sumTime / maxCount))
